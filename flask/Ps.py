...
def One_Move(ittCount, swarm, contact, pointCount, threshold,  outFilePtr, convFact, lossFunctionChoice):
...
def Optimize(inFilePtr, outFilePtr, convFact, constraint, points, zeroInd, randRange, swarmSize, threshold, ittCount, lossFunctionChoice):
    ...
    ittFin = One_Move(ittCount, swarm, constraint, len(points), threshold,  outFilePtr, convFact, lossFunctionChoice)
    return (
...
def Par_Choice(inFilePtr, outFilePtr, alpha):
    ...
    if 1==1:
        ...
        swarms = pool.starmap(
            Optimize,  
            zip(
                repeat(inFilePtr), repeat(outFilePtr), 
                alphas, repeat(contact),repeat(points),
                repeat(zeroInd), repeat(randRange),
                repeat(swarmSize), repeat(threshold),
                repeat(ittCount), repeat(lossFunctionChoice),
            )
        )
